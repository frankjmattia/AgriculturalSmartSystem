...
status_text = u_gui.draw_text(text="05_status：NAN",x=0,y=290,font_size=16, color="#0000FF")
...
def send_photos():
    ...
    # b64encode是编码，b64decode是解码 
        ...
        siot.publish_save(topic="siot/节点2/image", data=src)
        print("send ok")
        f.close()
        status_text.config(text="05_status: photo send ok",x=0,y=290)
            ...
            if uart1.read(1)[0] == 0x03:       
                ...
                if crc == '0x0':
                    ...
                    try :
                        ...
                        status_text.config(text="05_wifi:"+status,x=0,y=290)

                        siot.init(client_id="unihiker05",server="10.168.1.100",port=1883,user="siot",password="dfrobot")
                        siot.connect()
                        siot.loop()
                        siot.getsubscribe(topic="siot/节点5/土壤温度")
                        siot.getsubscribe(topic="siot/节点5/土壤湿度")
                        siot.getsubscribe(topic="siot/节点5/土壤pH")
                        siot.getsubscribe(topic="siot/节点5/土壤氮")
                        siot.getsubscribe(topic="siot/节点5/土壤磷")
                        siot.getsubscribe(topic="siot/节点5/土壤钾")
                        siot.getsubscribe(topic="siot/节点5/二氧化碳")
                        siot.getsubscribe(topic="siot/节点5/空气温度")
                        siot.getsubscribe(topic="siot/节点5/空气湿度")
                        siot.getsubscribe(topic="siot/节点5/光照强度")
                        siot.getsubscribe(topic="siot/image")
                        siot.getsubscribe(topic="siot/节点2/image")

                        siot.publish_save(topic="siot/节点5/土壤温度", data=soil_tem)
                        siot.publish_save(topic="siot/节点5/土壤pH", data=soil_ph)
                        siot.publish_save(topic="siot/节点5/土壤湿度", data=soil_hum)
                        siot.publish_save(topic="siot/节点5/土壤氮", data=soil_N)
                        siot.publish_save(topic="siot/节点5/土壤磷", data=soil_P)
                        siot.publish_save(topic="siot/节点5/土壤钾", data=soil_K)
                        siot.publish_save(topic="siot/节点5/二氧化碳", data=CO2_value)
                        siot.publish_save(topic="siot/节点5/空气温度", data=air_tem_value)
                        siot.publish_save(topic="siot/节点5/空气湿度", data=air_hum_value)
                        siot.publish_save(topic="siot/节点5/光照强度", data=light_value)
                        send_photos()
                        ...
                        print("send ok")
                        siot.stop()
                        status_text.config(text="05_status: data send ok",x=0,y=290)
                        time.sleep(6000)

                    except :        
                        print("wifi正在重连！")
                        status_text.config(text="05正在重连服务器！",x=0,y=290)

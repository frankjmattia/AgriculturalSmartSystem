...
status_text = u_gui.draw_text(text="03_status：NAN",x=0,y=290,font_size=16, color="#0000FF")
...
#发送给传感器的指令
#buf = [0x03, 0x03, 0x00, 0x00, 0x00, 0x0A, 0xC4,0x2F]
buf = [0x01, 0x03, 0x00, 0x00, 0x00, 0x0A, 0xC5, 0xCD]
...
def send_photos():
    ...
    # b64encode是编码，b64decode是解码 
        ...
        siot.publish_save(topic="siot/节点3/image", data=src)
        print("send ok")
        f.close()
        photos_count = int(photos_count) + 1
        status_text.config(text="03_status: photo send ok",x=0,y=290)
            ...
            if uart1.read(1)[0] == 0x03:       
                ...
                if crc == '0x0':
                    ...
                    try :
                        ...
                        status_text.config(text="03_wifi:"+status,x=0,y=290)

                        siot.init(client_id="unihiker03",server="10.168.1.100",port=1883,user="siot",password="dfrobot")
                        siot.connect()
                        siot.loop()

                        siot.getsubscribe(topic="siot/节点3/土壤温度")
                        siot.getsubscribe(topic="siot/节点3/土壤湿度")
                        siot.getsubscribe(topic="siot/节点3/土壤pH")
                        siot.getsubscribe(topic="siot/节点3/土壤氮")
                        siot.getsubscribe(topic="siot/节点3/土壤磷")
                        siot.getsubscribe(topic="siot/节点3/土壤钾")
                        siot.getsubscribe(topic="siot/节点3/二氧化碳")
                        siot.getsubscribe(topic="siot/节点3/空气温度")
                        siot.getsubscribe(topic="siot/节点3/空气湿度")
                        siot.getsubscribe(topic="siot/节点3/光照强度")
                        siot.getsubscribe(topic="siot/节点3/image")
                        siot.getsubscribe(topic="siot/image")

                        siot.publish_save(topic="siot/节点3/土壤温度", data=soil_tem)
                        siot.publish_save(topic="siot/节点3/土壤pH", data=soil_ph)
                        siot.publish_save(topic="siot/节点3/土壤湿度", data=soil_hum)
                        siot.publish_save(topic="siot/节点3/土壤氮", data=soil_N)
                        siot.publish_save(topic="siot/节点3/土壤磷", data=soil_P)
                        siot.publish_save(topic="siot/节点3/土壤钾", data=soil_K)
                        siot.publish_save(topic="siot/节点3/二氧化碳", data=CO2_value)
                        siot.publish_save(topic="siot/节点3/空气温度", data=air_tem_value)
                        siot.publish_save(topic="siot/节点3/空气湿度", data=air_hum_value)
                        siot.publish_save(topic="siot/节点3/光照强度", data=light_value)
                        ...
                        print("send ok")
                        siot.stop()
                        status_text.config(text="03_status: data send ok",x=0,y=290)
                        print(flag)
                        flag = flag + 1
                        time.sleep(3600)
                        ...
                        print("wifi正在重连！")
                        status_text.config(text="03_wifi正在重连！",x=0,y=290)
